...
* **SQD sub-jobs** -- for ``SQD`` fragments, one job per SBD batch and one
  final ext-SQD job:

  - ``<workdir>/sqd_scratch_<FFF>/iter_<CCC>/batch_<BBB>/sbd_job.{sh,status}``
    (job name ``sqd_i<CCC>_b<BBB>_batch_<BBB>``) -- ``n_batches`` per SQD
    recovery iteration, all submitted in parallel.
  - ``<workdir>/sqd_scratch_<FFF>/ext_sqd_iter/sbd_job.{sh,status}``
    (job name ``sqd_extsqd_ext_sqd_iter``) -- one final ext-SQD job that
    augments the recovered subspace with PyCI single excitations and runs
    SBD with ``--rdm 1`` to obtain the per-fragment 1- and 2-RDMs.

  Both SQD sub-job kinds use the same ``sqd.slurm.sbatch.mem`` knob.
...
_MEM_KNOB = {
    ...
    "solve:SQD":      "slurm.SQD.mem  (outer orchestrator job)",
    "sbd":            "sbd.slurm.sbatch.mem  (per-cycle SBD diagonalisation)",
    "sqd:batch":      "sqd.slurm.sbatch.mem  (per-batch SBD diagonalisation)",
    "sqd:extsqd":     "sqd.slurm.sbatch.mem  (final ext-SQD SBD diagonalisation)",
}
...
@dataclass
class Job:
    layer: str                      # 'dump' | 'solve' | 'sbd' | 'sqd'
    step: str = ""                  # 'step_000' or '' (single-point)
    frag: str = ""                  # fragment index (zero-padded) or ''
    cycle: str = ""                 # SBD growth cycle / SQD iter+batch / 'ext_sqd' / ''
    solver: str = ""                # FCI / SCI / SCI_SBD / SQD / batch / extsqd
    name: str = ""                  # Slurm --job-name                  # Slurm --job-name
    status_path: str = ""
    sh_path: str = ""
    out_path: str = ""
    err_path: str = ""
    ...
    for root, _dirs, files in os.walk(workdir):
        for fn in files:
            ...
            if fn == "sbd_job.status":
                # Disambiguate SCI_SBD vs SQD by the scratch-dir parent.
                if "sqd_scratch_" in root:
                    fm = re.search(r"sqd_scratch_(\d+)", root)
                    sh = os.path.join(root, "sbd_job.sh")
                    if "ext_sqd_iter" in root:
                        # Final ext-SQD job: one per fragment.
                        jobs.append(Job(
                            layer="sqd", step=step,
                            frag=fm.group(1) if fm else "",
                            cycle="ext_sqd",
                            solver="extsqd",
                            name=_job_name_from_sh(sh), status_path=spath,
                            sh_path=sh,
                            out_path=os.path.join(root, "slurm.out"),
                            err_path=os.path.join(root, "slurm.err"),
                            status_state=state, status_rc=rc,
                            inline_jobid=inline, anchor_epoch=anchor))
                    else:
                        # Per-batch SBD job inside an SQD iteration.
                        cm = re.search(r"iter_(\d+)", root)
                        bm = re.search(r"batch_(\d+)", root)
                        cycle = ""
                        if cm and bm:
                            cycle = f"iter_{cm.group(1)}/batch_{bm.group(1)}"
                        elif cm:
                            cycle = f"iter_{cm.group(1)}"
                        jobs.append(Job(
                            layer="sqd", step=step,
                            frag=fm.group(1) if fm else "",
                            cycle=cycle,
...
def _oom_reason(job, likely=False):
    ...
    if job.layer == "sbd":
        ...
        kind = ("per-batch SBD" if job.solver == "batch"
                else "final ext-SQD SBD" if job.solver == "extsqd"
                else "SQD sub-job")
        extra = (f"  NOTE: this is a {kind} job inside an SQD fragment -- its "
                 f"memory is sized by sqd.slurm.sbatch.mem, NOT the outer SQD "
                 f"orchestrator (slurm.SQD.mem).")
...
def main(argv=None):
    ...
    if not jobs:
        ...
        print("Has the workflow run yet?  Expected jobs_fragments_production/, "
              "jobs_ci_calculations/, sci_sbd_scratch_*/ and/or sqd_scratch_*/ "
              "subfolders.")
        return 0
